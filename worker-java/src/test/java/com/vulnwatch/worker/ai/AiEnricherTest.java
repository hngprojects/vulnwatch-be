...
class AiEnricherTest {
    ...
    @Mock
    private FindingsConverter findingsConverter;

    @Mock
    private ScoreCalculator scoreCalculator;

    @Mock
    private FallbackResultCreator fallbackCreator;
    ...
    void setUp() {
        ...
        ScanResult dnsResult = ScanResult.success(
                scanId, "DnsScanner", SurfaceType.DNS,
                Map.of("has_dmarc", false, "dnssec_enabled", false)
        );

        aggregatedData = AggregatedScanData.builder()
                .scanId(scanId)
                .scanJob(scanJob)
    ...
    void enrich_shouldHandleZeroFindingsSuccessfully() {
        // Given
        AiResponse emptyResponse = AiResponse.builder()
                .securityScore(100)
                .findings(List.of())
                .build();
    ...
    void enrich_shouldHandleNullFindingsList() {
        // Given
        AiResponse nullFindingsResponse = AiResponse.builder()
                .securityScore(100)
                .findings(null)
                .build();
    ...
    void enrich_shouldReturnFallbackResultWithCorrectFields_whenExceptionOccurs() {
        ...
        EnrichedScanResult expectedFallback = EnrichedScanResult.builder()
                .scanId(scanId)
                .securityScore(50)
                .findings(List.of())
                .processedAt(Instant.now())
    ...
    void enrich_shouldCallComponentsInCorrectOrder() {
        ...
        // Then
        InOrder inOrder = inOrder(promptBuilder, responseParser, findingsConverter, scoreCalculator, fallbackCreator);
    ...
    void enrich_shouldHandleNullScanJobInAggregatedData() {
        // Given
        AggregatedScanData dataWithNullScanJob = AggregatedScanData.builder()
                .scanId(scanId)
                .scanJob(null)
        ...
        when(promptBuilder.buildPrompt(dataWithNullScanJob)).thenThrow(new RuntimeException(errorMessage));
    ...
    void enrich_shouldHandleEmptySuccessfulResults() {
        // Given
        AggregatedScanData emptyData = AggregatedScanData.builder()
                .scanId(scanId)
                .scanJob(scanJob)
                .successfulResults(List.of())
                .failures(List.of())
