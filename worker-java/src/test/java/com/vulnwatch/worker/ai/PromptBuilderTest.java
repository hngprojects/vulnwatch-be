...
class PromptBuilderTest {
    ...
    void convertScanDataToJson_shouldGroupResultsBySurface() {
        ...
        ScanResult sslResult = ScanResult.success(
                scanId, "SslScanner", SurfaceType.SSL, Map.of("expiry_days", 25, "weak_protocols", List.of("TLSv1.0"))
        );

        AggregatedScanData aggregatedData = AggregatedScanData.builder()
                .scanId(scanId)
    ...
    void convertScanDataToJson_shouldHandleNullRawData() {
        ...
        ScanResult nullResult = ScanResult.builder()
                .scanId(scanId)
    ...
    void convertScanDataToJson_shouldPreserveOrderOfSurfaces() {
        ...
        ScanResult sslResult = ScanResult.success(
                scanId, "SslScanner", SurfaceType.SSL, Map.of("expiry_days", 25)
        );

        ScanResult httpResult = ScanResult.success(
    ...
    private AggregatedScanData createMockAggregatedData() {
        ...
        ScanResult dnsResult = ScanResult.success(
                scanId, "DnsScanner", SurfaceType.DNS, Map.of("has_dmarc", false, "dnssec_enabled", false)
        );
