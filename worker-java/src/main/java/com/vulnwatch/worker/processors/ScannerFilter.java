 ...
 *
 * <p>Since a scan job can request multiple target types (e.g., DOMAIN and REPOSITORY),
 * this class selects all scanners that support ANY of the requested types.
 */
@Slf4j
@Component
public class ScannerFilter {
     ...
     */
    public List<Scanner> filterByAnyTargetType(List<Scanner> allScanners, List<TargetType> jobTargetTypes) {
        ...
        if (log.isDebugEnabled()) {
            String scannerNames = matchingScanners.stream()
    ...
    /**
     * Returns the set of target types supported by the given scanners.
