...
public class SPFRules implements Rule{
    ...
    public List<Finding> evaluate(ScanContext context) {
        List<String> spfRecords = context.txtRecordList().stream()
        ...
        if (spfRecords.size() > 1) {
            ...
            if (spf.contains("+all")) {
                findings.add(Finding.high(
                        ...
                        "SPF uses +all (allows any sender)"
                ));
            }
            ...
            long includes = spf.chars()
                    .filter(c -> c == 'i')
                    .count(); // rough proxy; you can improve later

            if (includes > 10) {
                findings.add(Finding.medium(
                        "SPF_TOO_MANY_LOOKUPS",
                        "SPF may exceed RFC lookup limits"
                ));
            }
        }
