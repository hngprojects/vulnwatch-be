...
public class ttlRules implements Rule{
    ...
    public List<Finding> evaluate(ScanContext context) {
        ...
        ttlMap.forEach((k, v) -> {
            ...
            if (max-min>70000){
                findings.add(Finding.medium("TTL_CONFIGURATION", "Possible ttl misconfiguration in " +k));
            ...
            v.forEach( ttl -> {
                if (ttl>100000){
                    findings.add(Finding.high("TTL_CONFIGURATION", "Possible ttl misconfiguration in  " + k));
                }
                    }
            );
    ...
    private Map<String, Set<Long>> extractTTL(ScanContext context) {
        ...
        ttlMap.put("aRecord", context.aRecordList().stream().map(ARecord::getTTL).collect(Collectors.toSet()));
        ttlMap.put("aaaaRecord", context.aaaaRecordList().stream().map(AAAARecord::getTTL).collect(Collectors.toSet()));
        ttlMap.put("mxRecord", context.mxRecordList().stream().map(MXRecord::getTTL).collect(Collectors.toSet()));
