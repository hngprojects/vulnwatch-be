...
public class AggregatedScanData {
    ...
    @Schema(description = "Failed scanner results - each FAILURE PRESERVES SurfaceType so we know which scanner failed")
    ...
    public List<SurfaceType> getSuccessfulSurfaces() {
        ...
        return successfulResults.stream()
                .map(ScanResult::getSurface)
                .toList();
    }

    /**
     * Returns the set of surface types that failed.
     * This tells us EXACTLY which scanners failed (DNS, SSL, etc.)
     */
    public List<SurfaceType> getFailedSurfaces() {
        if (failures == null)
            return List.of();
        return failures.stream()
                .map(FailureInfo::getSurface)
                .toList();
    }
