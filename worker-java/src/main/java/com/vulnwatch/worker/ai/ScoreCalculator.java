...
/**
 * Calculates and validates security scores for scan results.
 * Uses hybrid approach: merges AI-provided score with rule-based calculation.
 *
 * <p>This component has NO external dependencies (no Redis, no Database, no HTTP).
 * It is purely a calculator
 */
@Slf4j
@Component
public class ScoreCalculator {
    ...
    public int calculateFinalScore(Integer aiScore, List<AiFinding> findings) {
        ...
        if (difference > ANOMALY_THRESHOLD) {
            log.warn("AI score ({}) differs significantly from rule score ({}). Difference: {}",
                    validatedAiScore, ruleScore, difference);
